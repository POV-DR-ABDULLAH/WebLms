...
const CourseDetails = () => {
  ...
  const {allCourses, calculateRating, calculateNoOfLectures, calculateTotalDuration, calculateChapterTime, currency} = useContext(AppContext)
  ...
  return courseData ? (
    <>
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left">
        ...
        <p className="text-sm">Course By <span className="text-blue-600 underline">Abdullah</span></p>
        ...
        <div className="max-w-[424px] z-10 shadow-[0px_4px_15px_2px_rgba(0,0,0,0.1)] rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
            ...
            <div className="p-5">
              <div className="flex items-center gap-2">
                ...
                <p className="text-red-500"><span className="font-medium">5 days</span> left this a price!</p>
              </div>

              <div className="flex items-center gap-3 pt-2">
                <p className="text-gray-800">{currency}{(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
                <p className="md:text-lg text-gray-500 line-through">{currency}{courseData.coursePrice}</p>
                <p className="md:text-lg text-blue-500">{courseData.discount}% off</p>
              </div>

              <div className="flex items-center text-sm md:text-default gap-2 pt-2 md:pt-4 text-gray-500">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <p className="">{calculateTotalDuration(courseData)}</p>
                </div>

                <div className="h-4 w-px bg-gray-500/40"></div>
                <div className="flex items-center gap-1">
                  <img src={assets.lesson_icon} alt="lesson icon" className="w-3.5" />
                  <p className="">{calculateNoOfLectures(courseData)} Lesson</p>
                </div>
              </div>
